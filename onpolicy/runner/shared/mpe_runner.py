...
class MPERunner(Runner):
    ...
    def run(self):
        ...
        for episode in range(episodes):
            ...
            dones_step = np.zeros(self.n_rollout_threads)
            ...
            for step in range(self.episode_length):
                ...
                if self.use_train_render:  #  and step < self.episode_length/4 and episode>episodes/2
                    # print("1111")
            ...
            for thread in range(self.n_rollout_threads):
                if dones_step[thread]:
                    continue
                dones_step[thread] = self.episode_length
    ...
    def render(self):
        ...
        def _extract_first_frame(render_out):
            """Normalize different rgb_array return shapes from vectorized envs."""
            if render_out is None:
                return None
            frame = render_out
            # Dummy/Subproc wrappers may return nested lists like [[img]] or [img].
            while isinstance(frame, list) and len(frame) > 0:
                frame = frame[0]
            if isinstance(frame, np.ndarray):
                # Some wrappers return (1, 1, H, W, C) or (1, H, W, C).
        ...
        for episode in range(self.all_args.render_episodes):
            ...
            if self.all_args.save_gifs:
                image = _extract_first_frame(envs.render('rgb_array'))
